...
const PATTERNS: Array<[RegExp, string]> = [
  ...
  [new RegExp(`("(?:${SENSITIVE_KEY_PATTERN})"\\s*:\\s*)"[^"]*"`, "gi"), '$1"[REDACTED]"'],
  // Header / query-string / kv form: key=value or key: value (quoted or not).
  [new RegExp(`\\b(${SENSITIVE_KEY_PATTERN})\\s*[=:]\\s*"?[^\\s,;"']+"?`, "gi"), "$1=[REDACTED]"]
];

export function redactProviderSecrets(input: string) {
  return PATTERNS.reduce((output, [pattern, replacement]) => output.replace(pattern, replacement), input);
}
