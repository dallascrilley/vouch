...
export function registerVerdictFeedbackRoutes(app: FastifyInstance) {
  app.get<{ Params: { jobId: string } }>(
    ...
    async (request, reply) => {
      ...
      const verdict = await app.services.verdictRepository.findByJobId(
  ...
  app.get<{ Params: { jobId: string } }>(
    ...
    async (request, reply) => {
      ...
      const signal = await app.services.feedbackRepository.findByJobId(
        request.params.jobId
      );
      if (!signal) {
      ...
      return {
        ...
        agent_next_action:
          signal.agentNextAction ?? deriveAgentNextAction(signal),
