...
function classifyUnanimousVerdict(
  response: HumanResponse
): UnanimousVerdict | null {
  if (
  ...
  if (
    ...
    response.criterionResults.every(
      (criterion) =>
        criterion.status === "fail" && criterion.confidence === "high"
    )
  ) {
    return "fail";
  }
...
export class ProviderWorkflowService {
  ...
  async maybeAutoAdvanceAfterIngest(input: {
    ...
    const siblingResponses = await this.responseRepository.findByReviewTaskId(
      input.reviewTaskId
    );
    if (
      !siblingResponses.every(
        (sibling) => classifyUnanimousVerdict(sibling) === unanimousVerdict
      )
    ) {
      return { advanced: false };
    }
    ...
    const resolvedBudgetPolicy = resolveBudgetPolicy(
      job.budgetPolicy,
      job.riskTier
    );
    const retryAllowed =
      unanimousVerdict === "fail" && resolvedBudgetPolicy.maxRetries > 0;
    ...
    await this.transactionManager.inTransaction(async () => {
      ...
      const verdict = await this.verdictService.finalize(
        ...
        unanimousVerdict,
        {
          ...
          criterionOutcomes:
            unanimousVerdict === "fail" ? response.criterionResults : [],
      ...
      await this.feedbackService.emit(currentJob, verdict, {
        defectCategory:
          unanimousVerdict === "fail" ? response.defectCategory : undefined,
  ...
  async maybeQueuePairwiseTieBreak(input: {
    ...
    const distinctVerdicts = [
      ...new Set(responses.map((response) => response.overallVerdict))
    ];
    ...
    if (
      responses.some((response) => SEVERE_SEVERITIES.has(response.severity))
    ) {
      return notQueued;
    }
    ...
    return this.transactionManager.inTransaction(async () => {
      const existingTasks = await this.reviewTaskRepository.findByJobId(
        job.jobId
      );
      if (
        existingTasks.some(
