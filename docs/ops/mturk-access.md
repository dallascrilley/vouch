...
| Requester portal (browser, rarely needed) | Sandbox: https://requestersandbox.mturk.com · Production: use **https://requester.mturk.com/begin_aws_signin** (creates/links prod MTurk to AWS `181596276354`) — **not** bare AWS sign-in on the homepage (that 403s or shows “no MTurk account”). Signs in with **AWS root** `dallas@dallascrilley.com` (passkey MFA required once). IAM users cannot use the portal. After link: fund via portal Billing or enable AWS Billing in console. Local helper: `bash scripts/ops/mturk-prod-aws-link.sh` (opens headed browser through MFA). |
