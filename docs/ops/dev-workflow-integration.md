...
- **Sim provider (default, no external provider):** with `LOCAL_PROVIDER_MODE` unset the broker resolves the escalation synchronously through the local provider simulator and the auto-advance pipeline, so `npm run verify` completes without a worker. Dogfood it end-to-end with `VERIFY_FORCE_HUMAN_REVIEW=true npm run verify` plus an injected failing check — failures are reported as `unclear` and the verdict closes via `verification.provider.auto_resolved`.
- **Crowd-provider path:** run against a deployed broker (`BROKER_URL`) with `PROVIDER_ENABLED=true` and the MTurk bridge. The gate blocks for up to `VERIFY_HITL_TIMEOUT_MS` waiting for the provider callback; on timeout it surfaces the job's stuck-state (`GET /verification-jobs/:jobId/stuck-state`) instead of hanging.
