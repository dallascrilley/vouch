...
export function buildMockProviderBridge(config: MockProviderBridgeConfig): FastifyInstance {
  ...
  app.post<{ Body: MockProviderResponseBody }>("/responses", async (request, reply) => {
    if (!isAuthorized(request.headers.authorization, config.apiKey)) {
      return reply.code(401).send({ message: "Invalid bridge authorization" });
    }
    ...
    if (!task) {
      return reply.code(404).send({ message: `Provider task not found: ${request.body.provider_task_id}` });
    }

    const responseId = request.body.provider_response_id ?? `mock_response_${crypto.randomUUID()}`;
...
function buildCallbackPayload(input: {
  ...
  return {
    ...
    defect_category: input.body.defect_category ?? (verdict === "pass" ? "none" : "mock_provider_review"),
    delivery_mode: "callback",
    evidence_note: input.body.evidence_note ?? `Mock provider response for ${input.task.reviewTaskId}.`,
    ...
    reviewer_pseudonymous_id: input.body.reviewer_pseudonymous_id ?? "mock-provider-worker",
